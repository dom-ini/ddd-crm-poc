...
def test_invalid_field_raises_exception(filter_service: FilterService, model: Model) -> None:
    filters = [
        FilterCondition(
            field="invalid field",
            value="some string",
    ...
    with pytest.raises(InvalidFilterField):
        filter_service.apply_filters(entity=model, filters=filters)


def test_invalid_condition_type_raises_exception(filter_service: FilterService, model: Model) -> None:
    filters = [
        FilterCondition(
            field="field_1",
            value="some string",
