...
from tests.infrastructure.sql.conftest import TEST_DB_URL


def test_get_db_session_returns_session() -> None:
    with get_db_session(TEST_DB_URL) as db:
...
def test_get_session_factory_does_not_recreate_factory_when_already_created(
    ...
    factory_1 = connection_manager.get_session_factory(TEST_DB_URL)
    factory_2 = connection_manager.get_session_factory(TEST_DB_URL)
