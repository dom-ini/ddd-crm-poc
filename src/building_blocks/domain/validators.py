...
AllowedValues = Iterable[ValueT]
GetDuplicatesCallback = Callable[[ValueT], Any]


def validate_no_duplicates(value: ValueT, callback: GetDuplicatesCallback = lambda x: x) -> None:
    duplicates = get_duplicates(value, callback)
    if duplicates:
        raise DuplicateEntry(duplicates)


def validate_value_in(value: ValueT, allowed_values: AllowedValues) -> None:
